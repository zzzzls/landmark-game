...
<!-- IMPLEMENTATION SNAPSHOT: src/styles.css, src/arcade.css, src/UI.jsx, src/PixelArt.jsx, src/Home.jsx, src/Invite.jsx, src/GameRoom.jsx, src/Screen.jsx and src/Results.jsx. Descriptive design documentation, not runtime acceptance evidence. -->
...
本文记录当前实现。基础 token 与组件值保留于 `src/styles.css`，`src/arcade.css` 在其后加载，定义城市挑战券的构图、层次和响应式覆盖；实际样式以两者的最终级联为准。入口是挑战券、三题是关卡、成绩是战绩票；定位针角色和原创北京城景贯穿入口与少人数大屏。
...
中文继续使用系统无衬线字体。首页桌面标题为 `clamp(52px, 5.7vw, 82px)`，手机为 `clamp(39px, 10.8vw, 48px)`，字重900；桌面行高1.16、手机1.08。手机短屏标题34px，城景隐藏以给表单留空间。任务标题桌面24px、手机22px；紧凑答题题名使用22px（≤380px为20px），不随短屏缩小。输入文字16px，正文15px。战绩总误差使用30–40px像素数字，单位用13px系统字体，名次数字34px。
...
城市挑战券在入口和大屏使用海军蓝描边与硬阴影，入口标题的金黄招牌形成主视觉。战绩票使用海军蓝票头、奶黄色票面、虚线票根和薄荷绿双线名次印章；头名在深海军蓝展示区中使用奶白姓名和金黄成绩。辅助列表保留轻描边，真实地图不添加背景纹理或装饰性动效。
...
- **Task feedback：** 三枚关卡保持原触控尺寸，底部金黄游标以180ms移动；服务端确认后的勾选短暂缩放。减少动态效果模式关闭两者。
